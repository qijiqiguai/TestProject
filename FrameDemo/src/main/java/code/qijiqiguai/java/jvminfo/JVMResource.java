package code.qijiqiguai.java.jvminfo;
...
public class JVMResource{
    ...
    public void printSummary() {
        ...
        for (long tId : threadIds) {
            ThreadInfo tInfo = threads.getThreadInfo(tId);
            System.out.println( "Thread: " + tInfo.getThreadName() );
        }

        Runtime imp = Runtime.getRuntime();
        System.out.println(imp.maxMemory());
        System.out.println(imp.freeMemory());


        // 操作系统级内存情况查询
        OperatingSystemMXBean osmxb = ManagementFactory.getOperatingSystemMXBean();
        double loadAverage = osmxb.getSystemLoadAverage();
        System.out.println("SystemLoad：" + loadAverage);

        SystemInfo si = new SystemInfo();
        si.getHardware().getMemory();
        si.getHardware().getSensors();
        System.out.println(si.getOperatingSystem().getProcess(1).getName());
    }
